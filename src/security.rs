...
pub fn decrypt(enc_password: String, user_id: i32) -> String {
    let home_dir = env::var("HOME").expect("Error: HOME environment variable not found");
    let priv_key = RsaPrivateKey::read_pkcs1_pem_file(format!("{}/.local/pm/keys/{}/private_key.pem", home_dir, user_id)).unwrap();
...
pub fn generateKeys(user_id: i32){
    let home_dir = env::var("HOME").expect("Error: HOME environment variable not found");
    if !path::Path::new(format!("{}/.local/pm/keys/{}", home_dir, user_id).as_str()).is_dir() {
        let bits = 2048;
        let priv_key = RsaPrivateKey::new(&mut OsRng, bits).expect("failed to generate a key");
        let pub_key = RsaPublicKey::from(&priv_key);
        if !path::Path::new(format!("{}/.local/pm/keys", home_dir).as_str()).is_dir() {
            fs::create_dir(format!("{}/.local/pm/keys", home_dir).as_str()).expect("cannot create keys folder");
            fs::create_dir("keys").expect("cannot create keys folder");
        }
        fs::create_dir(format!("{}/.local/pm/keys/{}", home_dir, user_id.to_string())).expect("cannot create user_id folder");
        ...
        priv_key.to_pkcs1_der().unwrap().write_pem_file(format!("{}/.local/pm/keys/{}/private_key.pem", home_dir, user_id.to_string()), "RSA PRIVATE KEY", LineEnding::LF)
            .expect("TODO: panic message");
    }
}
